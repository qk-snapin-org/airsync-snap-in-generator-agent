...
## Prerequisites
This command requires **two MCP servers**:
1. **Snap-in Builder MCP** (this plugin) — for guides, templates, validation, and code patterns
2. **DevRev AirSync MCP** (via `chef-cli`) — for metadata testing and field mapping

If the AirSync MCP is not connected, stop and tell the user to set it up:
